...
import 'package:app/admin/adminMainPage.dart';
...
import 'package:app/chats/components/deleteAndUpdateMessage.dart';
...
class _MyAppState extends State<MyApp> {
  static final navigator = VxNavigator(observers: [
    MyObs()
  ], routes: {
    '/': (_, __) {
      return MaterialPage(
          ...
          if (snapshot.connectionState == ConnectionState.done) {
            if (snapshot.hasData) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                context.vxNav.replace(Uri.parse((snapshot.data == false)
                    ? Routes.loginPage
                    : Routes.homeScreen));
              });
            }
    ...
    Routes.deleteAndUpdate: (uri, _) {
      final type = uri.queryParameters["type"];
      final id = uri.queryParameters["msgId"];
      final body = uri.queryParameters["body"];
      return MaterialPage(child: DeleteAndUpdate(type, id, body));
    },
